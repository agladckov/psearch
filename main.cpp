...
void computeLPSArray(unsigned int patlen, unsigned int* lps);

// Prints occurrences of txt[] in pat[]
void KMPSearch(const char* txt, const char* file, unsigned int txtlen)
{
    unsigned int patlen = strlen(filelist::pat);
    ...
    while (i < txtlen)
    {
        ...
        if (j == patlen)
        {
            ...
            if (strnum != lastprinted)
              printf("%s : %d : %.*s\n", file, strnum, strend - strbegin + 2, txt + strbegin - 1);
            lastprinted = strnum;
        		j = lps[j - 1];
        }
        // mismatch after j matches
        else if (i < txtlen && filelist::pat[j] != txt[i])
        {
...
void computeLPSArray(unsigned int M, unsigned int* lps)
{
    ...
    unsigned int i = 1;
    while (i < M) {
        if (filelist::pat[i] == filelist::pat[len]) {
            len++;
            lps[i] = len;
            i++;
        }
...
int searchinfile()
{
  ...
  while (not filelist::filequeue.empty())
  {
    ...
    KMPSearch((char *) memstart, curfile, fsize);
